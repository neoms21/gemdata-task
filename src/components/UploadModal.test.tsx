...
describe('UploadModal component', () => {
  ...
  it('shows error for invalid file name', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Unable to generate metadata as filename is not valid/i)).toBeInTheDocument();
    });
  });
  ...
  it('shows selected file details for valid file name', async () => {
    ...
    const fileInput = container.querySelector('input[type="file"]') as HTMLInputElement;
    const file = new File(['1,2,3'], 'equity_vanilla_option_hkg.csv', { type: 'text/csv' });
    await userEvent.upload(fileInput, file);
    
    await waitFor(() => {
      expect(screen.getByText('equity_vanilla_option_hkg.csv')).toBeInTheDocument();
    });
    expect(screen.queryByText(/Unable to generate metadata as filename is not valid/i)).not.toBeInTheDocument();
  ...
  it('submits file successfully and reloads page', async () => {
    ...
    server.use(
      http.post('http://localhost:3000/universeDefinitions', async ({ request }) => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: 'Confirm Upload' })).not.toBeDisabled();
    });
